...
- 🎯 Entry points to execute your program using `python -m <flask_demo>` or `$ flask_demo` with basic CLI argument parsing.
...
---
# flask_demo Flask Application

Awesome flask_demo created by teddyxiong53
...
```bash
git clone https://github.com/teddyxiong53/flask-demo flask_demo
cd flask_demo
make install
```
...
```bash
flask_demo create-db   # run once
flask_demo populate-db  # run once (optional)
flask_demo add-user -u admin -p 1234  # ads a user
flask_demo run
```
