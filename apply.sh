...
original_author="teddyxiong53"
original_name="flask_demo"
original_urlname="flask-demo"
original_description="Awesome flask_demo created by teddyxiong53" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_demo "${name}"
...
then
    ...
    echo "then run 'flask_demo' or 'python -m flask_demo'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_demo' or 'python -m flask_demo' or 'poetry run flask_demo'"
fi
